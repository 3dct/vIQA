    ...
    def set_centers(
        ...
        **kwargs : optional
        ...
        if img.ndim == 2:
            ...
            x_slider_bg = widgets.IntSlider(min=0, max=img.shape[0]-1, step=1, value=img.shape[0] // 2, description='X background', color='red')
            y_slider_bg = widgets.IntSlider(min=0, max=img.shape[1]-1, step=1, value=img.shape[1] // 2, description='Y background', color='green')
            radius_slider_bg = widgets.IntSlider(min=1, max=min(img.shape[0], img.shape[1], img.shape[2])-1, step=1, value=1, description='Radius', color='purple')
            x_slider_sig = widgets.IntSlider(min=0, max=img.shape[0]-1, step=1, value=img.shape[0] // 2, description='X signal', color='red')
            y_slider_sig = widgets.IntSlider(min=0, max=img.shape[1]-1, step=1, value=img.shape[1] // 2, description='Y signal', color='green')
            ...
            def update_visualization(x_bg, y_bg,radius, x_sig, y_sig):
                ...
                _visualize_cnr_2d(img=img, signal_center=signal_center, background_center=background_center, radius=radius, export_path=export_path, **kwargs)
            
            self._saved_parameters = {"signal_center": None, "background_center": None, "radius": None}
            ...
            def display_values(b):
                print("Displayed saved background center:", self._saved_parameters["background_center"])
                print("Displayed saved signal center:", self._saved_parameters["signal_center"])
            ...
            out = widgets.interactive_output(update_visualization,{'x_bg': x_slider_bg, 'y_bg': y_slider_bg,'radius':radius_slider_bg,'x_sig': x_slider_sig,'y_sig': y_slider_sig})

            display(out)
            display(VBox([HBox([x_slider_bg, y_slider_bg],layout=widgets.Layout(justify_content='space-between')),HBox([x_slider_sig, y_slider_sig,],layout=widgets.Layout(justify_content='space-between')),radius_slider_bg, HBox([save_button, display_button])]),layout=widgets.Layout(padding='10px 60px'))
            ...
            x_slider_bg = widgets.IntSlider(min=0, max=img.shape[0]-1, step=1, value=img.shape[0] // 2, description='X background', color='red')
            y_slider_bg = widgets.IntSlider(min=0, max=img.shape[1]-1, step=1, value=img.shape[1] // 2, description='Y background', color='green')
            z_slider_bg = widgets.IntSlider(min=0, max=img.shape[2]-1, step=1, value=img.shape[2] // 2, description='Z background', color='blue')
            ...
            x_slider_sig = widgets.IntSlider(min=0, max=img.shape[0]-1, step=1, value=img.shape[0] // 2, description='X signal', color='red')
            y_slider_sig = widgets.IntSlider(min=0, max=img.shape[1]-1, step=1, value=img.shape[1] // 2, description='Y signal', color='green')
            z_slider_sig = widgets.IntSlider(min=0, max=img.shape[2]-1, step=1, value=img.shape[2] // 2, description='Z signal', color='blue')
            ...
            def update_visualization(x_bg, y_bg, z_bg, radius, x_sig, y_sig, z_sig):
                ...
                _visualize_cnr_3d(img=img, signal_center=signal_center, background_center=background_center, radius=radius, export_path=export_path, **kwargs)
            ...
            out = widgets.interactive_output(update_visualization,{'x_bg': x_slider_bg, 'y_bg': y_slider_bg,'z_bg': z_slider_bg,'radius':radius_slider_bg,'x_sig': x_slider_sig,'y_sig': y_slider_sig,'z_sig': z_slider_sig})
            
            display(out)
            display(VBox([HBox([x_slider_bg, y_slider_bg,z_slider_bg],layout=widgets.Layout(justify_content='space-between')),HBox([x_slider_sig, y_slider_sig,z_slider_sig],layout=widgets.Layout(justify_content='space-between')),radius_slider_bg, HBox([save_button, display_button])]),layout=widgets.Layout(padding='10px 60px'))
